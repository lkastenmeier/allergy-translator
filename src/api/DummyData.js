export const allergyData = {
  milk: {
    images: {
      basic: "/images/milk.svg",
      warning: "/images/millkWarning.svg"
    },
    languages: {
      english:
        "I am allergic to milk - to prevent a severe allergic reaction, please make sure my food does not contain any milk!",
      german:
        "Ich bin allergisch gegen Milch. Um eine schwere allergische Reaktion zu vermeiden, stellen Sie bitte sicher, dass mein Essen keine Milch enthält.",
      french:
        "Je suis allergique au lait. Pour éviter une réaction allergique sévère, s'il vous plait soyez sûrs que ma nourriture ne contienne pas de lait.",
      spanish:
        "Soy alérgico a leche - para prevenir una reacción alérgica severa, por favor asegúrese de que mi comida no contenga leche!",
      romanian:
        "Sunt alergic/alergică la lapte - pentru a preveni o severă reacție alergică, vă rog să vă asigurați că mâncarea mea nu conține nici o urmă de lapte!",
      swedish:
        "Jag är allergisk mot mjölk - för att förhindra en allvarlig allergisk reaktion, vänligen se till att min mat inte innehåller mjölk!",
      dutch:
        "Ik ben allergisch voor melk- om een ​​ernstige allergische reactie te voorkomen, moet u ervoor zorgen dat mijn voedsel geen melk bevat!",
      nepali:
        "चेतावनी (मृत्युको खतरा): मैले दुग्ध पदार्थ अथवा यो भएको खाना एलर्जीका कारणले खान मिल्दैन। यदि खानामा रैछ भने एलर्जीले मरणाशन्न हुनेछु।",
      italian:
        "Io sono allergica al latte - Per evitare una grave reazione allergica devo essere sicura che la mia alimentazione non contenga questi prodotti!"
    }
  },

  soy: {
    images: { basic: "/images/soy.svg", warning: "/images/soyWarning.svg" },
    languages: {
      english:
        "I am allergic to soy - to prevent a severe allergic reaction, please make sure my food does not contain any soy!",
      german:
        "Ich bin allergisch gegen Soja. Um eine schwere allergische Reaktion zu vermeiden, stellen Sie bitte sicher, dass mein Essen kein Soja enthält.",
      french:
        "Je suis allergique au soja. Pour éviter une réaction allergique sévère, s'il vous plait soyez sûrs que ma nourriture ne contienne pas de soja.",
      spanish:
        "Soy alérgico a soja - para prevenir una reacción alérgica severa, por favor asegúrese de que mi comida no contenga soja!",
      romanian:
        "Sunt alergic/alergică la soia - pentru a preveni o severă reacție alergică, vă rog să vă asigurați că mâncarea mea nu conține nici o urmă de soia!",
      swedish:
        "Jag är allergisk mot soja - för att förhindra en allvarlig allergisk reaktion, vänligen se till att min mat inte innehåller soja!",
      dutch:
        "Ik ben allergisch voor soja- om een ​​ernstige allergische reactie te voorkomen, moet u ervoor zorgen dat mijn voedsel geen soja bevat!",
      nepali:
        "चेतावनी मृत्युको खतरा: मैले भटमास अथवा यो भएको खाना एलर्जीका कारणले खान मिल्दैन। यदि खानामा रैछ भने एलर्जीले मरणाशन्न हुनेछु।",
      italian:
        "Io sono allergica al soja - Per evitare una grave reazione allergica devo essere sicura che la mia alimentazione non contenga questi prodotti!"
    }
  },

  peanuts: {
    images: {
      basic: "/images/peanuts.svg",
      warning: "/images/peanutsWarning.svg"
    },
    languages: {
      english:
        "I am allergic to peanuts - to prevent a severe allergic reaction, please make sure my food does not contain any peanuts!",
      german:
        "Ich bin allergisch gegen Erdnüsse. Um eine schwere allergische Reaktion zu vermeiden, stellen Sie bitte sicher, dass mein Essen keine Erdnüsse enthält.",
      french:
        "Je suis allergique aux cacahuètes. Pour éviter une réaction allergique sévère, s'il vous plait soyez sûrs que ma nourriture ne contienne pas de cachuètes.",
      spanish:
        "Soy alérgico a cacahuetes - para prevenir una reacción alérgica severa, por favor asegúrese de que mi comida no contenga cacahuetes!",
      romanian:
        "Sunt alergic/alergică la alune - pentru a preveni o severă reacție alergică, vă rog să vă asigurați că mâncarea mea nu conține nici o urmă de alune!",
      swedish:
        "Jag är allergisk mot jordnötter - för att förhindra en allvarlig allergisk reaktion, vänligen se till att min mat inte innehåller jordnötter!",
      dutch:
        "Ik ben allergisch voor pindas- om een ​​ernstige allergische reactie te voorkomen, moet u ervoor zorgen dat mijn voedsel geen pinda bevat!",
      nepali:
        "चेतावनी मृत्युको खतरा: मैले बदाम अथवा यो भएको खाना एलर्जीका कारणले खान मिल्दैन। यदि खानामा रैछ भने एलर्जीले मरणाशन्न हुनेछु।",
      italian:
        "Io sono allergica al arachidi - Per evitare una grave reazione allergica devo essere sicura che la mia alimentazione non contenga questi prodotti!"
    }
  },
  treenuts: {
    images: {
      basic: "/images/treenuts.svg",
      warning: "/images/treenutsWarning.svg"
    },
    languages: {
      english:
        "I am allergic to tree nuts - to prevent a severe allergic reaction, please make sure my food does not contain any treenuts!",
      german:
        "Ich bin allergisch gegen Nüsse. Um eine schwere allergische Reaktion zu vermeiden, stellen Sie bitte sicher, dass mein Essen keine Nüsse enthält.",
      french:
        "Je suis allergique aux noix. Pour éviter une réaction allergique sévère, s'il vous plait soyez sûrs que ma nourriture ne contienne pas de noix.",
      spanish:
        "Soy alérgico a nueces - para prevenir una reacción alérgica severa, por favor asegúrese de que mi comida no contenga nueces!",
      romanian:
        "Sunt alergic/alergică la fructe cu coaja lemnoasă (nuci, migdale) - pentru a preveni o severă reacție alergică, vă rog să vă asigurați că mâncarea mea nu conține nici o urmă de fructe cu coaja lemnoasă (nuci, migdale)!",
      swedish:
        "Jag är allergisk mot trädnötter - för att förhindra en allvarlig allergisk reaktion, vänligen se till att min mat inte innehåller trädnötter!",
      dutch:
        "Ik ben allergisch voor soja- om een ​​ernstige allergische reactie te voorkomen, moet u ervoor zorgen dat mijn voedsel geen soja bevat!",
      nepali:
        "चेतावनी मृत्युको खतरा: मैले ओखर अथवा यो भएको खाना एलर्जीका कारणले खान मिल्दैन। यदि खानामा रैछ भने एलर्जीले मरणाशन्न हुनेछु।",
      italian:
        "Io sono allergica al nocciole - Per evitare una grave reazione allergica devo essere sicura che la mia alimentazione non contenga questi prodotti!"
    }
  },
  eggs: {
    images: {
      basic: "/images/eggs.svg",
      warning: "/images/eggsWarning.svg"
    },
    languages: {
      english:
        "I am allergic to tree eggs - to prevent a severe allergic reaction, please make sure my food does not contain any eggs!",
      german:
        "Ich bin allergisch gegen Eier. Um eine schwere allergische Reaktion zu vermeiden, stellen Sie bitte sicher, dass mein Essen keine Eier enthält.",
      french:
        "Je suis allergique aux œufs. Pour éviter une réaction allergique sévère, s'il vous plait soyez sûrs que ma nourriture ne contienne pas d'œufs.",
      spanish:
        "Soy alérgico a huevos - para prevenir una reacción alérgica severa, por favor asegúrese de que mi comida no contenga huevos!",
      romanian:
        "Sunt alergic/alergică la ouă - pentru a preveni o severă reacție alergică, vă rog să vă asigurați că mâncarea mea nu conține nici o urmă de ouă!",
      swedish:
        "Jag är allergisk mot ägg- för att förhindra en allvarlig allergisk reaktion, vänligen se till att min mat inte innehåller ägg!",
      dutch:
        "Ik ben allergisch voor eieren- om een ​​ernstige allergische reactie te voorkomen, moet u ervoor zorgen dat mijn voedsel geen eieren bevat!",
      nepali:
        "चेतावनी मृत्युको खतरा: मैले अण्डा अथवा यो भएको खाना एलर्जीका कारणले खान मिल्दैन। यदि खानामा रैछ भने एलर्जीले मरणाशन्न हुनेछु।",
      italian:
        "Io sono allergica alle uova - Per evitare una grave reazione allergica devo essere sicura che la mia alimentazione non contenga questi prodotti!"
    }
  },
  wheat: {
    images: {
      basic: "/images/wheat.svg",
      warning: "/images/wheatWarning.svg"
    },
    languages: {
      english:
        "I am allergic to tree wheat - to prevent a severe allergic reaction, please make sure my food does not contain any wheat!",
      german:
        "Ich bin allergisch gegen Weizen. Um eine schwere allergische Reaktion zu vermeiden, stellen Sie bitte sicher, dass mein Essen keinen Weizen enthält.",
      french:
        "Je suis allergique au blé. Pour éviter une réaction allergique sévère, s'il vous plait soyez sûrs que ma nourriture ne contienne pas de blé.",
      spanish:
        "Soy alérgico a trigo - para prevenir una reacción alérgica severa, por favor asegúrese de que mi comida no contenga trigo!",
      romanian:
        "Sunt alergic/alergică la cereale - pentru a preveni o severă reacție alergică, vă rog să vă asigurați că mâncarea mea nu conține nici o urmă de cereale!",
      swedish:
        "Jag är allergisk mot vete - för att förhindra en allvarlig allergisk reaktion, vänligen se till att min mat inte innehåller vete!",
      dutch:
        "Ik ben allergisch voor tarwe- om een ​​ernstige allergische reactie te voorkomen, moet u ervoor zorgen dat mijn voedsel geen tarwe bevat!",
      nepali:
        "चेतावनी मृत्युको खतरा: मैले गहुँ अथवा यो भएको खाना एलर्जीका कारणले खान मिल्दैन। यदि खानामा रैछ भने एलर्जीले मरणाशन्न हुनेछु।",
      italian:
        "Io sono allergica al grano - Per evitare una grave reazione allergica devo essere sicura che la mia alimentazione non contenga questi prodotti!"
    }
  },
  fish: {
    images: {
      basic: "/images/fish.svg",
      warning: "/images/fishWarning.svg"
    },
    languages: {
      english:
        "I am allergic to fish - to prevent a severe allergic reaction, please make sure my food does not contain any fish!",
      german:
        "Ich bin allergisch gegen Fisch. Um eine schwere allergische Reaktion zu vermeiden, stellen Sie bitte sicher, dass mein Essen keinen Fisch enthält.",
      french:
        "Je suis allergique au poisson. Pour éviter une réaction allergique sévère, s'il vous plait soyez sûrs que ma nourriture ne contienne pas de poisson.",
      spanish:
        "Soy alérgico a pescado - para prevenir una reacción alérgica severa, por favor asegúrese de que mi comida no contenga pescado!",
      romanian:
        "Sunt alergic/alergică la pește - pentru a preveni o severă reacție alergică, vă rog să vă asigurați că mâncarea mea nu conține nici o urmă de pește!",
      swedish:
        "Jag är allergisk mot fisk - för att förhindra en allvarlig allergisk reaktion, vänligen se till att min mat inte innehåller fisk!",
      dutch:
        "Ik ben allergisch voor vis- om een ​​ernstige allergische reactie te voorkomen, moet u ervoor zorgen dat mijn voedsel geen vis bevat!",
      nepali:
        "चेतावनी मृत्युको खतरा: मैले माछा अथवा यो भएको खाना एलर्जीका कारणले खान मिल्दैन। यदि खानामा रैछ भने एलर्जीले मरणाशन्न हुनेछु।",
      italian:
        "Io sono allergica al pesce - Per evitare una grave reazione allergica devo essere sicura che la mia alimentazione non contenga questi prodotti!"
    }
  },
  shellfish: {
    images: {
      basic: "/images/shellfish.svg",
      warning: "/images/shellfishWarning.svg"
    },
    languages: {
      english:
        "I am allergic to  shellfish - to prevent a severe allergic reaction, please make sure my food does not contain any  shellfish!",
      german:
        "Ich bin allergisch gegen Krustentiere. Um eine schwere allergische Reaktion zu vermeiden, stellen Sie bitte sicher, dass mein Essen keine Krustentiere enthält.",
      french:
        "Je suis allergique aux fruits de mer. Pour éviter une réaction allergique sévère, s'il vous plait soyez sûrs que ma nourriture ne contienne pas de fruits de mer.",
      spanish:
        "Soy alérgico a cascaras de marisco - para prevenir una reacción alérgica severa, por favor asegúrese de que mi comida no contenga mariscos!",
      romanian:
        "Sunt alergic/alergică la fructe de mare - pentru a preveni o severă reacție alergică, vă rog să vă asigurați că mâncarea mea nu conține nici o urmă de fructe de mare!",
      swedish:
        "Jag är allergisk mot skaldjur - för att förhindra en allvarlig allergisk reaktion, vänligen se till att min mat inte innehåller skaldjur!",
      dutch:
        "Ik ben allergisch voor schaaldieren- om een ​​ernstige allergische reactie te voorkomen, moet u ervoor zorgen dat mijn voedsel geen schaaldieren bevat!",
      nepali:
        "चेतावनी मृत्युको खतरा: मैले शेलफिश अथवा यो भएको खाना एलर्जीका कारणले खान मिल्दैन। यदि खानामा रैछ भने एलर्जीले मरणाशन्न हुनेछु।",
      italian:
        "Io sono allergica al frutti di mare - Per evitare una grave reazione allergica devo essere sicura che la mia alimentazione non contenga questi prodotti!"
    }
  }
};
